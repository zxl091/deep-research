...
const items = [
  { href: '/', label: '新的研究', icon: <EditOutlined /> },
  { href: '/knowledge', label: '知识库', icon: <BookOutlined /> },
  { href: '/memory', label: '会话与记忆', icon: <HistoryOutlined /> },
  { href: '/database', label: '数据库探索', icon: <DatabaseOutlined /> },
]
...
export function Nav({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return <>
    ...
    <nav className="workspace-nav" aria-label="主要功能">{items.map(item => <NavLink key={item.href} to={item.href} title={item.label} end={item.href === '/'} className={({ isActive }) => `workspace-nav-item ${isActive ? 'is-active' : ''}`}>{item.icon}<span>{item.label}</span></NavLink>)}</nav>
