...
class ContractTests(unittest.TestCase):
    def setUp(self):
        self.sources = {'https://example.test/demo': {'content': '虚构演示：2024年全年营收40亿元；2025年全年营收50亿元。'}}
        self.chart = {'title': '虚构演示营收', 'type': 'line', 'points': [dict(
            ...
            source_url='https://example.test/demo', quote=f'{year}年全年营收{value}亿元') for year, value in [(2024,40),(2025,50)]]}
    ...
    def test_typography_restores_actual_source_quote(self):
        source='2024 年，全年营收 40 亿元。'
        self.assertEqual(charts.original_quote('2024年,全年营收40亿元。',source),source)
        self.assertIsNone(charts.original_quote('2024年,全年营收55亿元。',source))

    def test_invented_value_or_quote_rejected(self):
        for key, value in [('value',55), ('quote','2025年全年营收55亿元'), ('source_url','https://unseen.test')]:
    ...
    def test_forecast_cannot_be_actual(self):
        item=copy.deepcopy(self.chart);item['points'][1]['quote']='预计2025年全年营收50亿元'
        self.sources['https://example.test/demo']['content'] += '预计2025年全年营收50亿元'
        self.assertFalse(charts.validate_charts([item],self.sources)[0])
