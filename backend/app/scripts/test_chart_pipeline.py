...
class PipelineTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        specs,self.sources=chart_fixtures();self.line=copy.deepcopy(specs[0]);self.bar=copy.deepcopy(specs[1])
        self.deep={'query':'虚构验收','outline':[{'id':'s1','title':'趋势'},{'id':'s2','title':'排名'}],'charts':[],'facts':[],
            'chart_plan':[{'id':'a','section_id':'s1','title':'趋势','type':'line','data_question':'趋势','desired_points':3},
                          {'id':'b','section_id':'s2','title':'排名','type':'bar','data_question':'排名','desired_points':3}]}
        self.execute=AsyncMock(return_value={'success':True,'charts':['fixture-image']})

    async def test_one_success_does_not_skip_other_chart_repair(self):
        bad=copy.deepcopy(self.bar);bad['points'][0]['quote']='无原文支持的错误摘录'
        model=type('Gateway',(),{})();model.complete=AsyncMock(side_effect=[{'chart':self.line},{'chart':bad,'search_queries':['查排名完整原文']},{'chart':self.bar}])
        supplement=AsyncMock(return_value=list(self.sources))
        await grounded_charts(self.deep,self.sources,model,self.execute,supplement=supplement)
        self.assertEqual(len(self.deep['charts']),2);self.assertEqual(model.complete.await_count,3)
        supplement.assert_awaited_once();self.assertEqual(self.execute.await_count,2)

    async def test_failed_followup_does_not_erase_good_chart(self):
        self.deep['charts']=[{'id':'old','plan_id':'a','verified_data':True,'data_contract':self.line,'image_base64':'old'}]
        model=type('Gateway',(),{})();model.complete=AsyncMock(return_value={'chart':None,'search_queries':['缺失数据']})
        supplement=AsyncMock(side_effect=RuntimeError('fixture transport failure'))
        await grounded_charts(self.deep,self.sources,model,self.execute,supplement=supplement)
        self.assertEqual(self.deep['charts'][0]['id'],'old');self.assertEqual(len(self.deep['charts']),1)
        self.assertEqual(self.deep['chart_validation']['items'][-1]['status'],'insufficient_data')

    async def test_render_failure_does_not_block_next_chart(self):
        model=type('Gateway',(),{})();model.complete=AsyncMock(side_effect=[{'chart':self.line},{'chart':self.bar}])
        self.execute.side_effect=[RuntimeError('sandbox fixture failure'),{'success':True,'charts':['fixture-image']}]
        await grounded_charts(self.deep,self.sources,model,self.execute)
        self.assertEqual(len(self.deep['charts']),1)
        self.assertEqual(self.deep['charts'][0]['plan_id'],'b')
        self.assertEqual(self.deep['chart_validation']['items'][0]['status'],'render_failed')

    async def test_supplemented_originals_are_available_to_second_extraction(self):
        self.deep['chart_plan']=self.deep['chart_plan'][1:]
        originals=copy.deepcopy(self.sources)
        for p in self.bar['points']:self.sources.pop(p['source_url'])
        async def supplement(*args):self.sources.update(originals);return [p['source_url'] for p in self.bar['points']]
        model=type('Gateway',(),{})();model.complete=AsyncMock(side_effect=[{'chart':self.bar},{'chart':self.bar}])
        await grounded_charts(self.deep,self.sources,model,self.execute,supplement=supplement)
        self.assertEqual(len(self.deep['charts']),1);self.assertEqual(model.complete.await_count,2)

    async def test_cancel_is_not_swallowed_as_missing_data(self):
        model=type('Gateway',(),{})();model.complete=AsyncMock(return_value={'chart':None,'search_queries':['缺失数据']})
        with self.assertRaises(asyncio.CancelledError):
            await grounded_charts(self.deep,self.sources,model,self.execute,supplement=AsyncMock(side_effect=asyncio.CancelledError()))

    async def test_two_point_fallback_keeps_visible_gap(self):
        self.deep['chart_plan']=self.deep['chart_plan'][:1]
        self.line['points']=self.line['points'][:2]
        model=type('Gateway',(),{})();model.complete=AsyncMock(return_value={'chart':self.line})
        await grounded_charts(self.deep,self.sources,model,self.execute)
        self.assertIn('未达到计划',self.deep['charts'][0]['data_contract']['coverage_note'])
        self.assertIn('不连线',self.deep['charts'][0]['data_contract']['display_note'])

    async def test_planner_binds_charts_to_actual_sections(self):
        self.deep.pop('chart_plan')
        url=self.line['points'][0]['source_url']
        model=type('Gateway',(),{})();model.complete=AsyncMock(side_effect=[{'chart_plan':[{'id':'provided','section_id':'s1','title':'趋势','type':'line','desired_points':3,'source_urls':[url,'https://invented.invalid'],'value_kind':'actual'}]},{'chart':self.line}])
        await grounded_charts(self.deep,self.sources,model,self.execute)
        self.assertEqual(self.deep['charts'][0]['section_id'],'s1')
        self.assertEqual(self.deep['charts'][0]['chart_type'],'line')
        self.assertEqual(self.deep['chart_plan'][0]['source_urls'],[url])

    async def test_actual_plan_cannot_be_satisfied_by_forecasts(self):
        self.deep['chart_plan']=self.deep['chart_plan'][:1]
        self.deep['chart_plan'][0]['value_kind']='actual'
        for p in self.line['points']:p['value_kind']='forecast'
        model=type('Gateway',(),{})();model.complete=AsyncMock(return_value={'chart':self.line})
        await grounded_charts(self.deep,self.sources,model,self.execute)
