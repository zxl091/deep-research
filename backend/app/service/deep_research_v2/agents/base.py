    ...
    async def call_llm(
        ...
        try:
            ...
            if json_mode:
                ...
                kwargs['messages'][0]['content'] += '\n请仅返回一个合法的 JSON 对象，不要添加 Markdown 代码围栏。'
            if self.model.startswith(('glm-', 'deepseek-v4-', 'qwen3.7-')):
                kwargs['extra_body'] = {'enable_thinking': os.getenv('RESEARCH_ENABLE_THINKING', 'false').lower() == 'true'}
            ...
            if not content.strip():
                raise RuntimeError(f'模型返回空正文（finish_reason={response.choices[0].finish_reason}），请检查输出额度与思考模式')
