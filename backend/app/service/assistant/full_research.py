...
def export_report(text, evidence):
    ...
    text = re.sub(r'(?<!!)\[([^\]]+)\]\(([^)]+)\)',
                  lambda m: m[1] + f'[{urls[m[2]]}]' if m[2] in urls else m[1] + '（来源未核验）', text)
...
def source_context(deep, snapshots, limit=40):
    """优先本轮补查和正文实际引用，避免审核永远只看到最早的事实。"""
    urls = []
    def add(url):
    ...
    for url in re.findall(r'\]\(([^)]+)\)', deep.get('final_report', '')):
        add(url)
    return '\n'.join(f"URL: {url}\n原始检索摘录: {snapshots[url].get('content', '')[:1800]}"
...
async def run_full_research(state, query, uid, sid, model, commit):
    ...
    agents = {name: cls(config.api_key, config.base_url, **({'search_api_key': config.search_api_key} if name == 'scout' else {}), model=config.default_model)
    ...
    def sync():
        ...
        deep['facts'] = [f for f in deep.get('facts', []) if f.get('source_url') in observed]
        deep['messages'] = []
    ...
    async def search(source, question, count=10, **kwargs):
        ...
        try:
            ...
            for row in rows:
                state['source_snapshots'][row['url']] = {'title': row.get('title', '资料'), 'url': row['url'],
    ...
    async def database_research():
        ...
        for section in deep['outline']:
            ...
            try:
                ...
                for item in results:
                    state['source_snapshots'][item['url']] = dict(item, retrieved_at=datetime.utcnow().isoformat())
        ...
        while state['next_stage'] != 'finish':
            ...
            if current == 'plan':
                ...
                if not deep['facts']:
                    ...
                    await stage('analyst', 'analyzing'); following = 'charts'
            elif current == 'charts':
